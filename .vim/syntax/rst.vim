...
syn match rstCharacter /^\s*\([-*+]\|#\.\)\s/me=e-1
syn match rstCharacter /[^\\]:$/ms=s+1
...
syn cluster rstDirectives           contains=rstFootnote,rstCitation,
      \ rstHyperlinkTarget,rstCode,rstExDirective
...
execute 'syn region rstExDirective contained matchgroup=rstDirective' .
      ...
      \ ' end=+$+ contains=@rstCruft'

execute 'syn region rstExDirective contained matchgroup=rstDirective' .
      \ ' start=+' . s:ReferenceName . '::\(\s\+\w\+\)\?\_s\+\n\z(\s\+\)+' .
      \ ' skip=+^$+' .
      \ ' end=+^\z1\@!+ contains=@rstCruft'

execute 'syn region rstCode contained matchgroup=rstDirective' .
      \ ' start=+code-block::\s\+\w\+\_s\+\n\z(\s\+\)+' .
      \ ' skip=+^$+' .
      \ ' end=+^\z1\@!+'
...
hi def link rstCharacter                    Character
