...
applyMagnifier z r _ wrs = do
    focused <- withWindowSet (return . peek)
    case focused of
        Just window -> do
            owner <- withDisplay (\d -> liftIO $ getTransientForHint d window)
            let mag (w,wr) ws | window == w || owner == Just w =
                                    ws ++ [(w, fit r $ magnify z wr)]
                              | otherwise = (w,wr) : ws
            return (reverse $ foldr mag [] wrs, Nothing)
        _ -> return (wrs, Nothing)
